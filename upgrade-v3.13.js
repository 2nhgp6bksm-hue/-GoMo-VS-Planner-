...
/*
  GoMo VS Planner v3.13 — Correctif boutons Safari
...
(() => {
  ...
  const KEY = 'gomo_vs_planner_v313';
  const OCR_SAFE = 70;
  ...
  function css(){
    if(document.getElementById('gomo-v313-style'))return;
    const s=document.createElement('style');s.id='gomo-v313-style';
    s.textContent=`
      #plannerView.gomo-v313-simple > :not(#gomoV313Guide){display:none!important}
      #gomoV300Assistant,#gomoV310Guide,#gomoV311Guide,#gomoV312Guide{display:none!important}
      #gomoV313Guide{display:block!important;position:relative!important;z-index:2147483000!important;pointer-events:auto!important}
      #gomoV313Guide *{pointer-events:auto}
      body.gomo-v313-simple .main-nav{display:none!important}
      body.gomo-v313-simple .nav-btn{display:none!important}
      .v313 button,.v313 select,.v313 input,.v313 summary{touch-action:manipulation;-webkit-tap-highlight-color:rgba(255,255,255,.08)}
      /* Compatibilité v3.00 : sa règle !important masquait le nouveau guide. */
      #plannerView.gomo-v300-simple.gomo-v313-simple > #gomoV313Guide{display:block!important}
      #plannerView.gomo-v300-simple.gomo-v313-simple > #gomoV300Assistant{display:none!important}
      .v313{max-width:760px;margin:0 auto;padding:8px 0 30px;color:#f7fbff}
      .v313-card{border:2px solid rgba(66,194,244,.58);border-radius:28px;background:linear-gradient(150deg,rgba(15,50,75,.98),rgba(5,26,43,.99));padding:20px;box-shadow:0 18px 50px rgba(0,0,0,.22)}
      .v313-center{text-align:center}.v313-brand{font-size:.78rem;font-weight:900;letter-spacing:.18em;color:#80dbff;margin:0 0 8px}.v313 h2{font-size:clamp(2rem,8vw,3rem);line-height:1.02;margin:0 0 12px}.v313 h3{font-size:1.45rem;line-height:1.15;margin:0 0 8px}.v313 p{line-height:1.52;margin:0;color:#cbe5f2}
      .v313-promise{font-size:1.12rem;max-width:520px;margin:0 auto!important}.v313-help{font-size:.9rem;margin-top:9px!important;color:#a9cad9!important}
      .v313-primary,.v313-secondary,.v313-ghost{width:100%;border:0;border-radius:20px;min-height:62px;padding:14px 18px;font:inherit;font-size:1.08rem;font-weight:900;cursor:pointer}
      .v313-primary{background:#26a7df;color:#fff;box-shadow:0 10px 24px rgba(38,167,223,.20)}.v313-secondary{background:#0c2b40;color:#d8f3ff;border:1px solid rgba(93,201,244,.38)}.v313-ghost{min-height:46px;background:transparent;color:#9dddf8;border:1px solid rgba(93,201,244,.22);font-size:.9rem}
      .v313-btns{display:grid;gap:10px;margin-top:18px}.v313-split{display:grid;grid-template-columns:1fr 1fr;gap:10px}.v313-split>.v313-secondary,.v313-split>.v313-primary{min-height:54px;font-size:.95rem}
      .v313-progress{display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;margin-bottom:14px}.v313-progress small{font-weight:900;color:#9dddf8}.v313-track{height:8px;border-radius:999px;background:#082337;overflow:hidden}.v313-fill{height:100%;background:#29a9df;border-radius:999px;transition:width .2s}
      .v313-day{display:grid;place-items:center;min-height:140px;border-radius:23px;margin-top:16px;background:rgba(37,168,224,.10);border:1px solid rgba(85,208,255,.28)}.v313-day strong{font-size:1.8rem;text-align:center;padding:10px}
      .v313-field{display:grid;gap:7px;margin-top:16px;text-align:left}.v313-field span{font-size:.86rem;font-weight:900;color:#c6e7f6}.v313-field input,.v313-field select,.v313-number{width:100%;box-sizing:border-box;border:2px solid rgba(104,198,235,.46);border-radius:18px;background:#071f31;color:#fff;padding:15px 16px;font:inherit;font-size:1.08rem;font-weight:900;min-height:56px}
      .v313-choices{display:grid;gap:12px;margin-top:18px}.v313-choice{display:grid;grid-template-columns:48px 1fr;gap:12px;align-items:center;border:2px solid rgba(77,192,235,.42);border-radius:22px;background:#092a40;color:#fff;padding:15px;text-align:left;font:inherit;font-weight:900;font-size:1.06rem}.v313-choice b{font-size:1.55rem}.v313-choice small{display:block;color:#a9cad9;font-size:.78rem;font-weight:700;margin-top:3px}
      .v313-upload{display:grid;place-items:center;min-height:92px;border:2px dashed rgba(83,208,255,.6);border-radius:22px;background:rgba(38,167,223,.08);color:#dcf7ff;font-size:1.08rem;font-weight:900;margin-top:16px;cursor:pointer}.v313-upload input{display:none}
      .v313-review{display:grid;gap:9px;margin-top:15px}.v313-row{display:grid;grid-template-columns:1fr 128px;gap:10px;align-items:center;border:1px solid rgba(108,190,225,.20);border-radius:17px;background:#09263a;padding:11px 12px}.v313-row strong{display:block;font-size:.9rem}.v313-row small{color:#85bed8}.v313-row input{width:100%;box-sizing:border-box;border:1px solid rgba(97,198,237,.35);border-radius:12px;background:#061c2c;color:#fff;padding:10px;font:inherit;font-weight:900}
      .v313-manual{display:grid;gap:8px;margin-top:15px}.v313-manual-row{display:grid;grid-template-columns:36px 1fr 118px;gap:9px;align-items:center;border-bottom:1px solid rgba(114,192,224,.14);padding:9px 0}.v313-manual-row .ico{font-size:1.25rem}.v313-manual-row strong{font-size:.9rem}.v313-manual-row small{display:block;color:#88b6ca;font-size:.72rem}.v313-manual-row input{width:100%;box-sizing:border-box;border:1px solid rgba(97,198,237,.35);border-radius:12px;background:#061c2c;color:#fff;padding:10px;font:inherit;font-weight:900}
      .v313-score{font-size:1.55rem!important;text-align:center;letter-spacing:.02em}
      .v313-stat{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:end;border-radius:20px;background:#08253a;border:1px solid rgba(95,191,229,.22);padding:14px;margin-top:10px}.v313-stat span{color:#9bc8da;font-size:.8rem;font-weight:800}.v313-stat strong{font-size:1.32rem}.v313-stat.gold strong{color:#ffd36a}
      .v313-alert{border-radius:20px;padding:15px;margin-top:14px;font-weight:900;line-height:1.45}.v313-ok{background:rgba(55,191,122,.13);border:1px solid rgba(92,222,153,.35);color:#9bf0bf}.v313-warn{background:rgba(255,173,65,.10);border:1px solid rgba(255,193,99,.34);color:#ffd08d}
      .v313-steps{counter-reset:plan;display:grid;gap:10px;margin-top:14px}.v313-planline{counter-increment:plan;display:grid;grid-template-columns:42px 1fr auto;gap:10px;align-items:center;border-radius:18px;background:#08253a;border:1px solid rgba(94,190,227,.20);padding:12px}.v313-planline:before{content:counter(plan);display:grid;place-items:center;width:34px;height:34px;border-radius:50%;background:#27a7df;color:#fff;font-weight:900}.v313-planline strong{display:block}.v313-planline small{display:block;color:#91bed1;margin-top:3px}.v313-gain{font-weight:900;color:#83ddff;white-space:nowrap}
      .v313-stop{font-size:1.08rem;text-align:center;margin-top:16px!important;color:#a6efc3!important;font-weight:900}
      .v313-done-icon{font-size:3rem;margin-bottom:8px}.v313-version{text-align:center;color:#6f9aae;font-size:.72rem;margin-top:12px}
      .v313-advanced{margin-top:14px;border-top:1px solid rgba(109,189,222,.17);padding-top:12px}.v313-advanced summary{cursor:pointer;color:#7fcce9;font-weight:800;font-size:.82rem}.v313-advanced p{font-size:.78rem;margin-top:8px!important}
      .v313-lang{margin:16px auto 0;max-width:240px}.v313-lang select{width:100%;border:1px solid rgba(96,198,237,.30);border-radius:14px;background:#071f31;color:#dff7ff;padding:10px 12px;font:inherit;font-weight:800}
      @media(max-width:520px){.v313-card{padding:18px 14px;border-radius:24px}.v313-split{grid-template-columns:1fr}.v313-row{grid-template-columns:1fr 110px}.v313-manual-row{grid-template-columns:32px 1fr 105px}.v313-planline{grid-template-columns:38px 1fr}.v313-gain{grid-column:2;text-align:left}}
    `;
  ...
  function ensure(){
    ...
    ['gomoV300Assistant','gomoV310Guide','gomoV311Guide','gomoV312Guide'].forEach(id=>{
      const old=document.getElementById(id);
      if(old) old.remove();
    });
    pv.classList.remove('gomo-v300-simple','gomo-v310-simple','gomo-v311-simple','gomo-v312-simple');
    let root=document.getElementById('gomoV313Guide');
    if(!root){root=document.createElement('section');root.id='gomoV313Guide';pv.insertBefore(root,pv.firstChild);}
    pv.classList.toggle('gomo-v313-simple',!ui.advanced);
    document.body?.classList.toggle('gomo-v313-simple',!ui.advanced);
    if(!ui.advanced) pv.classList.remove('gomo-v300-simple','gomo-v310-simple','gomo-v311-simple','gomo-v312-simple');
    return root;
  }
  ...
  function progress(step){
    ...
    return `<div class="v313-progress"><small>${esc(tr('step'))} ${Math.max(1,step)}</small><div class="v313-track"><div class="v313-fill" style="width:${pct}%"></div></div><small>4</small></div>`;
  }

  function backButton(to){
    return `<button class="v313-ghost" type="button" data-go="${to}">← ${esc(tr('back'))}</button>`;
  }

  function advancedBlock(){
    return `<details class="v313-advanced"><summary>⚙️ ${esc(tr('advanced'))}</summary><p>${esc(tr('advancedText'))}</p><div class="v313-btns"><button class="v313-ghost" id="v313AdvancedToggle" type="button">${esc(ui.advanced?tr('hideOld'):tr('showOld'))}</button></div></details>`;
  }

  function render(){
    const root=ensure();if(!root)return;
    root.innerHTML=`<div class="v313">${screen()}</div>`;
    bind(root);
  }
  ...
  function screenHome(){
    ...
    return `<div class="v313-card v313-center">
      <p class="v313-brand">GoMo Forever · LAST WAR</p>
      <h2>${esc(tr('title'))}</h2>
      <p class="v313-promise">${esc(tr('promise'))}</p>
      <p class="v313-help">${esc(tr('homeHelp'))}</p>
      <div class="v313-btns"><button class="v313-primary" id="v313Start" type="button">${esc(hasProgress?tr('resume'):tr('start'))}</button></div>
      <label class="v313-lang"><select id="v313Language" aria-label="${esc(tr('language'))}">${languageOptions()}</select></label>
      ${advancedBlock()}
      <div class="v313-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  }
  ...
  function screenDay(){
    return `<div class="v313-card">
      ${progress(1)}
      <h3>${esc(tr('dayTitle'))}</h3><p>${esc(tr('dayHelp'))}</p>
      <div class="v313-day"><strong>${esc(dayName(state.selectedDay))}</strong></div>
      ${ui.changeDay?`<label class="v313-field"><span>${esc(tr('chooseDay'))}</span><select id="v313Day">${dayOptions()}</select></label>`:''}
      <div class="v313-btns">
        <button class="v313-primary" id="v313DayOk" type="button">${esc(tr('yesContinue'))}</button>
        <button class="v313-secondary" id="v313ChangeDay" type="button">${esc(ui.changeDay?tr('continue'):tr('chooseDay'))}</button>
        ${backButton(0)}
      </div>
    </div>`;
  }
  ...
  function screenResources(){
    ...
    if(ui.inputMode==='choice'){
      body=`<div class="v313-choices">
        <button class="v313-choice" id="v313CaptureChoice" type="button"><b>📷</b><span>${esc(tr('addCaptures'))}<small>${esc(tr('captureHint'))}</small></span></button>
        <button class="v313-choice" id="v313ManualChoice" type="button"><b>✍️</b><span>${esc(tr('manual'))}<small>${esc(tr('manualHelp'))}</small></span></button>
      </div>`;
    ...
    return `<div class="v313-card">
      ...
      ${body}
      <div class="v313-btns">${ui.inputMode!=='choice'?`<button class="v313-secondary" id="v313ResourceChoice" type="button">${esc(tr('change'))}</button>`:''}${backButton(1)}</div>
    </div>`;
  }
  ...
  function captureMarkup(){
    ...
    if(busy)return `<label class="v313-upload">${esc(tr('reading'))}</label>`;
    if(!rows.length)return `<label class="v313-upload">📷 ${esc(tr('addCaptures'))}<input id="v313Files" type="file" accept="image/*" multiple></label><p class="v313-help">${esc(tr('captureHint'))}</p>`;
    return `<h3 style="margin-top:18px">${esc(tr('recognized'))}</h3><p class="v313-help">${esc(tr('recognizedHelp'))}</p>
      <div class="v313-review">${rows.map(({r,index})=>`<label class="v313-row"><span><strong>${esc(targetLabel(r.target))}</strong><small>${num(r.confidence||0)}% ${esc(tr('confidence'))}</small></span><input type="number" inputmode="decimal" min="0" data-ocr="${index}" value="${Number(r.value)||0}"></label>`).join('')}</div>
      <div class="v313-btns"><button class="v313-primary" id="v313ApplyOcr" type="button">${esc(tr('validate'))}</button><label class="v313-secondary" style="display:grid;place-items:center;cursor:pointer">📷 ${esc(tr('addCaptures'))}<input id="v313Files" type="file" accept="image/*" multiple style="display:none"></label></div>`;
  }

  function manualMarkup(){
    const rows=todayResources();
    return `<h3 style="margin-top:18px">${esc(tr('manualTitle'))}</h3><p class="v313-help">${esc(tr('manualHelp'))}</p>
      <div class="v313-manual">${rows.map(i=>`<label class="v313-manual-row"><span class="ico">${icon(i)}</span><span><strong>${esc(label(i))}</strong><small>${esc(unit(i)||tr('quantity'))}</small></span><input type="number" inputmode="decimal" min="0" data-stock="${esc(i.stockKey)}" value="${stock(i)}"></label>`).join('')}</div>
      <div class="v313-btns"><button class="v313-primary" id="v313ManualSave" type="button">${esc(tr('saveContinue'))}</button></div>`;
  }
  ...
  function screenScore(){
    ...
    return `<div class="v313-card">
      ...
      <label class="v313-field"><span>${esc(tr('currentScore'))}</span><input class="v313-score" id="v313Score" type="number" inputmode="numeric" min="0" value="${cur||''}" placeholder="${esc(tr('scorePlaceholder'))}"></label>
      <div class="v313-btns"><button class="v313-primary" id="v313MakePlan" type="button">${esc(tr('makePlan'))}</button>${backButton(2)}</div>
    </div>`;
  }
  ...
  function screenPlan(){
    ...
    const lines=p.resources.map(r=>`<div class="v313-planline"><span><strong>${esc(label(r.item))}</strong><small>${esc(formatQty(r.qty,r.item))}</small></span><span class="v313-gain">+${compact(r.points)}</span></div>`).join('');
    const status=p.current>=MINIMUM
      ? `<div class="v313-alert v313-ok">${esc(tr('noSpend'))}</div>`
      : p.reached
        ? `<div class="v313-alert v313-ok">${esc(tr('stop'))}</div>`
        : `<div class="v313-alert v313-warn">${esc(tr('insufficient'))}<br>${esc(tr('missing'))} ${compact(p.missingMinimum)}.</div>`;

    return `<div class="v313-card">
      ${progress(4)}
      <h3>${esc(tr('planTitle'))}</h3>
      <div class="v313-stat"><span>${esc(tr('already'))}</span><strong>${compact(p.current)}</strong></div>
      ${p.current<MINIMUM?`<div class="v313-stat gold"><span>${esc(tr('missing'))}</span><strong>${compact(need)}</strong></div>`:''}
      ${p.resources.length?`<h3 style="margin-top:20px">${esc(tr('exact'))}</h3><div class="v313-steps">${lines}</div>`:''}
      <div class="v313-stat gold"><span>${esc(tr('estimated'))}</span><strong>${compact(p.finalPoints)}</strong></div>
      <div class="v313-split" style="margin-top:10px"><div class="v313-stat"><span>${esc(tr('minimum'))}</span><strong>7,2 M</strong></div><div class="v313-stat"><span>${esc(tr('target'))}</span><strong>7,3 M</strong></div></div>
      ${status}
      <div class="v313-btns">
        ${p.resources.length?`<button class="v313-secondary" id="v313Copy" type="button">📋 ${esc(tr('copy'))}</button>`:''}
        <button class="v313-primary" id="v313Done" type="button">${esc(tr('done'))}</button>
        <div class="v313-split"><button class="v313-ghost" type="button" data-go="2">${esc(tr('editResources'))}</button><button class="v313-ghost" type="button" data-go="3">${esc(tr('editScore'))}</button></div>
      </div>
      ${advancedBlock()}
      <div class="v313-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  ...
  function screenFinished(){
    ...
    return `<div class="v313-card v313-center">
      <div class="v313-done-icon">✅</div><h2>${esc(tr('finishedTitle'))}</h2><p class="v313-promise">${esc(tr('finishedText'))}</p>
      <label class="v313-field"><span>${esc(tr('actualScore'))}</span><input class="v313-score" id="v313ActualScore" type="number" inputmode="numeric" min="0" value="${actual||''}"></label>
      <div class="v313-btns"><button class="v313-primary" id="v313SaveActual" type="button">${esc(tr('saveActual'))}</button><button class="v313-secondary" id="v313Restart" type="button">${esc(tr('reset'))}</button></div>
      ${advancedBlock()}
      <div class="v313-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  }
  ...
  function saveScore(root){
    const input=root.querySelector('#v313Score');const n=Math.max(0,Number(input?.value)||0);
  ...
  function saveActual(root){
    const n=Math.max(0,Number(root.querySelector('#v313ActualScore')?.value)||0);
  ...
  function bind(root){
    ...
    on('#v313Start','onclick',()=>goTo(1));

    on('#v313DayOk','onclick',()=>{
      ui.step=2;ui.inputMode='choice';ui.changeDay=false;saveUi();render();
    });

    on('#v313ChangeDay','onclick',()=>{
      if(ui.changeDay){ui.step=2;ui.inputMode='choice';ui.changeDay=false;}
      else ui.changeDay=true;
      saveUi();render();
    });

    on('#v313CaptureChoice','onclick',()=>{ui.inputMode='capture';saveUi();render();});
    on('#v313ManualChoice','onclick',()=>{ui.inputMode='manual';saveUi();render();});
    on('#v313ResourceChoice','onclick',()=>{ui.inputMode='choice';saveUi();render();});
    on('#v313ApplyOcr','onclick',()=>applyOcr(root));
    on('#v313ManualSave','onclick',()=>saveManual(root));
    on('#v313MakePlan','onclick',()=>saveScore(root));
    on('#v313Copy','onclick',()=>void copyPlan());
    on('#v313Done','onclick',()=>markDone());
    on('#v313SaveActual','onclick',()=>saveActual(root));
    on('#v313Restart','onclick',()=>restart());

    on('#v313AdvancedToggle','onclick',()=>{
      ui.advanced=!ui.advanced;
      saveUi();
      render();
    });

    on('#v313Language','onchange',e=>setLanguage(e.target.value));

    on('#v313Day','onchange',e=>{
    ...
    on('#v313Files','onchange',e=>void handleFiles(e.target.files));
  }
