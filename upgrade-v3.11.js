...
/*
  GoMo VS Planner v3.11 — Guidage automatique corrigé
...
(() => {
  ...
  const KEY = 'gomo_vs_planner_v311';
  const OCR_SAFE = 70;
  ...
  function css(){
    if(document.getElementById('gomo-v311-style'))return;
    const s=document.createElement('style');s.id='gomo-v311-style';
    s.textContent=`
      #plannerView.gomo-v311-simple > :not(#gomoV311Guide){display:none!important}
      #gomoV300Assistant{display:none!important}
      #gomoV311Guide{display:block!important}
      /* Compatibilité v3.00 : sa règle !important masquait le nouveau guide. */
      #plannerView.gomo-v300-simple.gomo-v311-simple > #gomoV311Guide{display:block!important}
      #plannerView.gomo-v300-simple.gomo-v311-simple > #gomoV300Assistant{display:none!important}
      .v311{max-width:760px;margin:0 auto;padding:8px 0 30px;color:#f7fbff}
      .v311-card{border:2px solid rgba(66,194,244,.58);border-radius:28px;background:linear-gradient(150deg,rgba(15,50,75,.98),rgba(5,26,43,.99));padding:20px;box-shadow:0 18px 50px rgba(0,0,0,.22)}
      .v311-center{text-align:center}.v311-brand{font-size:.78rem;font-weight:900;letter-spacing:.18em;color:#80dbff;margin:0 0 8px}.v311 h2{font-size:clamp(2rem,8vw,3rem);line-height:1.02;margin:0 0 12px}.v311 h3{font-size:1.45rem;line-height:1.15;margin:0 0 8px}.v311 p{line-height:1.52;margin:0;color:#cbe5f2}
      .v311-promise{font-size:1.12rem;max-width:520px;margin:0 auto!important}.v311-help{font-size:.9rem;margin-top:9px!important;color:#a9cad9!important}
      .v311-primary,.v311-secondary,.v311-ghost{width:100%;border:0;border-radius:20px;min-height:62px;padding:14px 18px;font:inherit;font-size:1.08rem;font-weight:900;cursor:pointer}
      .v311-primary{background:#26a7df;color:#fff;box-shadow:0 10px 24px rgba(38,167,223,.20)}.v311-secondary{background:#0c2b40;color:#d8f3ff;border:1px solid rgba(93,201,244,.38)}.v311-ghost{min-height:46px;background:transparent;color:#9dddf8;border:1px solid rgba(93,201,244,.22);font-size:.9rem}
      .v311-btns{display:grid;gap:10px;margin-top:18px}.v311-split{display:grid;grid-template-columns:1fr 1fr;gap:10px}.v311-split>.v311-secondary,.v311-split>.v311-primary{min-height:54px;font-size:.95rem}
      .v311-progress{display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;margin-bottom:14px}.v311-progress small{font-weight:900;color:#9dddf8}.v311-track{height:8px;border-radius:999px;background:#082337;overflow:hidden}.v311-fill{height:100%;background:#29a9df;border-radius:999px;transition:width .2s}
      .v311-day{display:grid;place-items:center;min-height:140px;border-radius:23px;margin-top:16px;background:rgba(37,168,224,.10);border:1px solid rgba(85,208,255,.28)}.v311-day strong{font-size:1.8rem;text-align:center;padding:10px}
      .v311-field{display:grid;gap:7px;margin-top:16px;text-align:left}.v311-field span{font-size:.86rem;font-weight:900;color:#c6e7f6}.v311-field input,.v311-field select,.v311-number{width:100%;box-sizing:border-box;border:2px solid rgba(104,198,235,.46);border-radius:18px;background:#071f31;color:#fff;padding:15px 16px;font:inherit;font-size:1.08rem;font-weight:900;min-height:56px}
      .v311-choices{display:grid;gap:12px;margin-top:18px}.v311-choice{display:grid;grid-template-columns:48px 1fr;gap:12px;align-items:center;border:2px solid rgba(77,192,235,.42);border-radius:22px;background:#092a40;color:#fff;padding:15px;text-align:left;font:inherit;font-weight:900;font-size:1.06rem}.v311-choice b{font-size:1.55rem}.v311-choice small{display:block;color:#a9cad9;font-size:.78rem;font-weight:700;margin-top:3px}
      .v311-upload{display:grid;place-items:center;min-height:92px;border:2px dashed rgba(83,208,255,.6);border-radius:22px;background:rgba(38,167,223,.08);color:#dcf7ff;font-size:1.08rem;font-weight:900;margin-top:16px;cursor:pointer}.v311-upload input{display:none}
      .v311-review{display:grid;gap:9px;margin-top:15px}.v311-row{display:grid;grid-template-columns:1fr 128px;gap:10px;align-items:center;border:1px solid rgba(108,190,225,.20);border-radius:17px;background:#09263a;padding:11px 12px}.v311-row strong{display:block;font-size:.9rem}.v311-row small{color:#85bed8}.v311-row input{width:100%;box-sizing:border-box;border:1px solid rgba(97,198,237,.35);border-radius:12px;background:#061c2c;color:#fff;padding:10px;font:inherit;font-weight:900}
      .v311-manual{display:grid;gap:8px;margin-top:15px}.v311-manual-row{display:grid;grid-template-columns:36px 1fr 118px;gap:9px;align-items:center;border-bottom:1px solid rgba(114,192,224,.14);padding:9px 0}.v311-manual-row .ico{font-size:1.25rem}.v311-manual-row strong{font-size:.9rem}.v311-manual-row small{display:block;color:#88b6ca;font-size:.72rem}.v311-manual-row input{width:100%;box-sizing:border-box;border:1px solid rgba(97,198,237,.35);border-radius:12px;background:#061c2c;color:#fff;padding:10px;font:inherit;font-weight:900}
      .v311-score{font-size:1.55rem!important;text-align:center;letter-spacing:.02em}
      .v311-stat{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:end;border-radius:20px;background:#08253a;border:1px solid rgba(95,191,229,.22);padding:14px;margin-top:10px}.v311-stat span{color:#9bc8da;font-size:.8rem;font-weight:800}.v311-stat strong{font-size:1.32rem}.v311-stat.gold strong{color:#ffd36a}
      .v311-alert{border-radius:20px;padding:15px;margin-top:14px;font-weight:900;line-height:1.45}.v311-ok{background:rgba(55,191,122,.13);border:1px solid rgba(92,222,153,.35);color:#9bf0bf}.v311-warn{background:rgba(255,173,65,.10);border:1px solid rgba(255,193,99,.34);color:#ffd08d}
      .v311-steps{counter-reset:plan;display:grid;gap:10px;margin-top:14px}.v311-planline{counter-increment:plan;display:grid;grid-template-columns:42px 1fr auto;gap:10px;align-items:center;border-radius:18px;background:#08253a;border:1px solid rgba(94,190,227,.20);padding:12px}.v311-planline:before{content:counter(plan);display:grid;place-items:center;width:34px;height:34px;border-radius:50%;background:#27a7df;color:#fff;font-weight:900}.v311-planline strong{display:block}.v311-planline small{display:block;color:#91bed1;margin-top:3px}.v311-gain{font-weight:900;color:#83ddff;white-space:nowrap}
      .v311-stop{font-size:1.08rem;text-align:center;margin-top:16px!important;color:#a6efc3!important;font-weight:900}
      .v311-done-icon{font-size:3rem;margin-bottom:8px}.v311-version{text-align:center;color:#6f9aae;font-size:.72rem;margin-top:12px}
      .v311-advanced{margin-top:14px;border-top:1px solid rgba(109,189,222,.17);padding-top:12px}.v311-advanced summary{cursor:pointer;color:#7fcce9;font-weight:800;font-size:.82rem}.v311-advanced p{font-size:.78rem;margin-top:8px!important}
      .v311-lang{margin:16px auto 0;max-width:240px}.v311-lang select{width:100%;border:1px solid rgba(96,198,237,.30);border-radius:14px;background:#071f31;color:#dff7ff;padding:10px 12px;font:inherit;font-weight:800}
      @media(max-width:520px){.v311-card{padding:18px 14px;border-radius:24px}.v311-split{grid-template-columns:1fr}.v311-row{grid-template-columns:1fr 110px}.v311-manual-row{grid-template-columns:32px 1fr 105px}.v311-planline{grid-template-columns:38px 1fr}.v311-gain{grid-column:2;text-align:left}}
    `;
  ...
  function ensure(){
    ...
    let root=document.getElementById('gomoV311Guide');
    if(!root){root=document.createElement('section');root.id='gomoV311Guide';pv.insertBefore(root,pv.firstChild);}
    pv.classList.toggle('gomo-v311-simple',!ui.advanced);
  ...
  function progress(step){
    ...
    return `<div class="v311-progress"><small>${esc(tr('step'))} ${Math.max(1,step)}</small><div class="v311-track"><div class="v311-fill" style="width:${pct}%"></div></div><small>4</small></div>`;
  }

  function backButton(to){
    return `<button class="v311-ghost" type="button" data-go="${to}">← ${esc(tr('back'))}</button>`;
  }

  function advancedBlock(){
    return `<details class="v311-advanced"><summary>⚙️ ${esc(tr('advanced'))}</summary><p>${esc(tr('advancedText'))}</p><div class="v311-btns"><button class="v311-ghost" id="v311AdvancedToggle" type="button">${esc(ui.advanced?tr('hideOld'):tr('showOld'))}</button></div></details>`;
  }

  function render(){
    const root=ensure();if(!root)return;
    root.innerHTML=`<div class="v311">${screen()}</div>`;
    bind(root);
  }
  ...
  function screenHome(){
    ...
    return `<div class="v311-card v311-center">
      <p class="v311-brand">GoMo Forever · LAST WAR</p>
      <h2>${esc(tr('title'))}</h2>
      <p class="v311-promise">${esc(tr('promise'))}</p>
      <p class="v311-help">${esc(tr('homeHelp'))}</p>
      <div class="v311-btns"><button class="v311-primary" id="v311Start" type="button">${esc(hasProgress?tr('resume'):tr('start'))}</button></div>
      <label class="v311-lang"><select id="v311Language" aria-label="${esc(tr('language'))}">${languageOptions()}</select></label>
      ${advancedBlock()}
      <div class="v311-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  }
  ...
  function screenDay(){
    return `<div class="v311-card">
      ${progress(1)}
      <h3>${esc(tr('dayTitle'))}</h3><p>${esc(tr('dayHelp'))}</p>
      <div class="v311-day"><strong>${esc(dayName(state.selectedDay))}</strong></div>
      ${ui.changeDay?`<label class="v311-field"><span>${esc(tr('chooseDay'))}</span><select id="v311Day">${dayOptions()}</select></label>`:''}
      <div class="v311-btns">
        <button class="v311-primary" id="v311DayOk" type="button">${esc(tr('yesContinue'))}</button>
        <button class="v311-secondary" id="v311ChangeDay" type="button">${esc(ui.changeDay?tr('continue'):tr('chooseDay'))}</button>
        ${backButton(0)}
      </div>
    </div>`;
  }
  ...
  function screenResources(){
    ...
    if(ui.inputMode==='choice'){
      body=`<div class="v311-choices">
        <button class="v311-choice" id="v311CaptureChoice" type="button"><b>📷</b><span>${esc(tr('addCaptures'))}<small>${esc(tr('captureHint'))}</small></span></button>
        <button class="v311-choice" id="v311ManualChoice" type="button"><b>✍️</b><span>${esc(tr('manual'))}<small>${esc(tr('manualHelp'))}</small></span></button>
      </div>`;
    ...
    return `<div class="v311-card">
      ...
      ${body}
      <div class="v311-btns">${ui.inputMode!=='choice'?`<button class="v311-secondary" id="v311ResourceChoice" type="button">${esc(tr('change'))}</button>`:''}${backButton(1)}</div>
    </div>`;
  }
  ...
  function captureMarkup(){
    ...
    if(busy)return `<label class="v311-upload">${esc(tr('reading'))}</label>`;
    if(!rows.length)return `<label class="v311-upload">📷 ${esc(tr('addCaptures'))}<input id="v311Files" type="file" accept="image/*" multiple></label><p class="v311-help">${esc(tr('captureHint'))}</p>`;
    return `<h3 style="margin-top:18px">${esc(tr('recognized'))}</h3><p class="v311-help">${esc(tr('recognizedHelp'))}</p>
      <div class="v311-review">${rows.map(({r,index})=>`<label class="v311-row"><span><strong>${esc(targetLabel(r.target))}</strong><small>${num(r.confidence||0)}% ${esc(tr('confidence'))}</small></span><input type="number" inputmode="decimal" min="0" data-ocr="${index}" value="${Number(r.value)||0}"></label>`).join('')}</div>
      <div class="v311-btns"><button class="v311-primary" id="v311ApplyOcr" type="button">${esc(tr('validate'))}</button><label class="v311-secondary" style="display:grid;place-items:center;cursor:pointer">📷 ${esc(tr('addCaptures'))}<input id="v311Files" type="file" accept="image/*" multiple style="display:none"></label></div>`;
  }

  function manualMarkup(){
    const rows=todayResources();
    return `<h3 style="margin-top:18px">${esc(tr('manualTitle'))}</h3><p class="v311-help">${esc(tr('manualHelp'))}</p>
      <div class="v311-manual">${rows.map(i=>`<label class="v311-manual-row"><span class="ico">${icon(i)}</span><span><strong>${esc(label(i))}</strong><small>${esc(unit(i)||tr('quantity'))}</small></span><input type="number" inputmode="decimal" min="0" data-stock="${esc(i.stockKey)}" value="${stock(i)}"></label>`).join('')}</div>
      <div class="v311-btns"><button class="v311-primary" id="v311ManualSave" type="button">${esc(tr('saveContinue'))}</button></div>`;
  }
  ...
  function screenScore(){
    ...
    return `<div class="v311-card">
      ...
      <label class="v311-field"><span>${esc(tr('currentScore'))}</span><input class="v311-score" id="v311Score" type="number" inputmode="numeric" min="0" value="${cur||''}" placeholder="${esc(tr('scorePlaceholder'))}"></label>
      <div class="v311-btns"><button class="v311-primary" id="v311MakePlan" type="button">${esc(tr('makePlan'))}</button>${backButton(2)}</div>
    </div>`;
  }
  ...
  function screenPlan(){
    ...
    const lines=p.resources.map(r=>`<div class="v311-planline"><span><strong>${esc(label(r.item))}</strong><small>${esc(formatQty(r.qty,r.item))}</small></span><span class="v311-gain">+${compact(r.points)}</span></div>`).join('');
    const status=p.current>=MINIMUM
      ? `<div class="v311-alert v311-ok">${esc(tr('noSpend'))}</div>`
      : p.reached
        ? `<div class="v311-alert v311-ok">${esc(tr('stop'))}</div>`
        : `<div class="v311-alert v311-warn">${esc(tr('insufficient'))}<br>${esc(tr('missing'))} ${compact(p.missingMinimum)}.</div>`;

    return `<div class="v311-card">
      ${progress(4)}
      <h3>${esc(tr('planTitle'))}</h3>
      <div class="v311-stat"><span>${esc(tr('already'))}</span><strong>${compact(p.current)}</strong></div>
      ${p.current<MINIMUM?`<div class="v311-stat gold"><span>${esc(tr('missing'))}</span><strong>${compact(need)}</strong></div>`:''}
      ${p.resources.length?`<h3 style="margin-top:20px">${esc(tr('exact'))}</h3><div class="v311-steps">${lines}</div>`:''}
      <div class="v311-stat gold"><span>${esc(tr('estimated'))}</span><strong>${compact(p.finalPoints)}</strong></div>
      <div class="v311-split" style="margin-top:10px"><div class="v311-stat"><span>${esc(tr('minimum'))}</span><strong>7,2 M</strong></div><div class="v311-stat"><span>${esc(tr('target'))}</span><strong>7,3 M</strong></div></div>
      ${status}
      <div class="v311-btns">
        ${p.resources.length?`<button class="v311-secondary" id="v311Copy" type="button">📋 ${esc(tr('copy'))}</button>`:''}
        <button class="v311-primary" id="v311Done" type="button">${esc(tr('done'))}</button>
        <div class="v311-split"><button class="v311-ghost" type="button" data-go="2">${esc(tr('editResources'))}</button><button class="v311-ghost" type="button" data-go="3">${esc(tr('editScore'))}</button></div>
      </div>
      ${advancedBlock()}
      <div class="v311-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  ...
  function screenFinished(){
    ...
    return `<div class="v311-card v311-center">
      <div class="v311-done-icon">✅</div><h2>${esc(tr('finishedTitle'))}</h2><p class="v311-promise">${esc(tr('finishedText'))}</p>
      <label class="v311-field"><span>${esc(tr('actualScore'))}</span><input class="v311-score" id="v311ActualScore" type="number" inputmode="numeric" min="0" value="${actual||''}"></label>
      <div class="v311-btns"><button class="v311-primary" id="v311SaveActual" type="button">${esc(tr('saveActual'))}</button><button class="v311-secondary" id="v311Restart" type="button">${esc(tr('reset'))}</button></div>
      ${advancedBlock()}
      <div class="v311-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  }
  ...
  function saveScore(root){
    const input=root.querySelector('#v311Score');const n=Math.max(0,Number(input?.value)||0);
  ...
  function saveActual(root){
    const n=Math.max(0,Number(root.querySelector('#v311ActualScore')?.value)||0);
  ...
  function bind(root){
    if(root.dataset.v311Bound==='1')return;
    root.dataset.v311Bound='1';
    root.addEventListener('click',e=>{
      const go=e.target.closest('[data-go]');if(go){ui.step=Number(go.dataset.go)||0;ui.finished=false;saveUi();render();return;}
      if(e.target.closest('#v311Start')){ui.step=1;ui.finished=false;saveUi();render();return;}
      if(e.target.closest('#v311DayOk')){ui.step=2;ui.inputMode='choice';ui.changeDay=false;saveUi();render();return;}
      if(e.target.closest('#v311ChangeDay')){
        if(ui.changeDay){ui.step=2;ui.inputMode='choice';ui.changeDay=false;}else ui.changeDay=true;
        saveUi();render();return;
      }
      if(e.target.closest('#v311CaptureChoice')){ui.inputMode='capture';saveUi();render();return;}
      if(e.target.closest('#v311ManualChoice')){ui.inputMode='manual';saveUi();render();return;}
      if(e.target.closest('#v311ResourceChoice')){ui.inputMode='choice';saveUi();render();return;}
      if(e.target.closest('#v311ApplyOcr')){applyOcr(root);return;}
      if(e.target.closest('#v311ManualSave')){saveManual(root);return;}
      if(e.target.closest('#v311MakePlan')){saveScore(root);return;}
      if(e.target.closest('#v311Copy')){void copyPlan();return;}
      if(e.target.closest('#v311Done')){markDone();return;}
      if(e.target.closest('#v311SaveActual')){saveActual(root);return;}
      if(e.target.closest('#v311Restart')){restart();return;}
      if(e.target.closest('#v311AdvancedToggle')){ui.advanced=!ui.advanced;saveUi();ensure();render();return;}
    });
    root.addEventListener('change',e=>{
      if(e.target.id==='v311Language'){setLanguage(e.target.value);return;}
      if(e.target.id==='v311Day'){
        state.selectedDay=Number(e.target.value);state.autoDay=false;planCache=null;persistBase();render();return;
      }
      if(e.target.id==='v311Files'){void handleFiles(e.target.files);return;}
    });
  }
  ...
  function start(){
    ...
    if(pv){
      const observer=new MutationObserver(()=>{
        if(!ui.advanced && pv.classList.contains('gomo-v300-simple')){
          pv.classList.remove('gomo-v300-simple');
        }
      });
