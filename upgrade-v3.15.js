...
/*
  GoMo VS Planner v3.15 — OCR inventaire sécurisé
...
(() => {
  ...
  const KEY = 'gomo_vs_planner_v315';
  const OCR_SAFE = 70;
  ...
  const T = {
    fr: {
      ...
      captureSafety:'Les captures servent uniquement à lire ton inventaire. Les actions, combats et ton score VS seront traités séparément.',
      ignoredActions:'Actions / combats détectés : ignorés automatiquement. Ils sont déjà compris dans ton score VS à l’étape suivante.',
      ignoredLow:'Lecture(s) sous 70 % ignorée(s). Reprends une capture plus nette ou utilise la saisie manuelle.',
      ignoredImplausible:'Valeur(s) manifestement anormale(s) ignorée(s) pour éviter un mauvais calcul.',
      safeContinue:'Continuer avec les valeurs sûres →',
      noSafeValues:'Aucune ressource d’inventaire suffisamment sûre n’a été reconnue. Ajoute une capture plus nette ou passe en saisie manuelle.'
    },
    en: {
      ...
      captureSafety:'Screenshots are used only to read inventory. Actions, combat and your VS score are handled separately.',
      ignoredActions:'Detected actions/combat values were ignored automatically. They are already included in your VS score at the next step.',
      ignoredLow:'Reading(s) below 70% were ignored. Use a clearer screenshot or manual entry.',
      ignoredImplausible:'Obviously abnormal value(s) were ignored to prevent a wrong calculation.',
      safeContinue:'Continue with safe values →',
      noSafeValues:'No sufficiently reliable inventory resource was recognised. Add a clearer screenshot or use manual entry.'
    },
    de: {
      ...
      captureSafety:'Screenshots lesen nur dein Inventar. Aktionen, Kämpfe und dein VS-Punktestand werden getrennt behandelt.',
      ignoredActions:'Erkannte Aktionen/Kampfwerte wurden automatisch ignoriert. Sie sind im VS-Punktestand des nächsten Schritts enthalten.',
      ignoredLow:'Messung(en) unter 70 % wurden ignoriert. Nutze einen klareren Screenshot oder die manuelle Eingabe.',
      ignoredImplausible:'Offensichtlich unplausible Werte wurden ignoriert, um falsche Berechnungen zu vermeiden.',
      safeContinue:'Mit sicheren Werten fortfahren →',
      noSafeValues:'Keine ausreichend sichere Inventarressource erkannt. Nutze einen klareren Screenshot oder die manuelle Eingabe.'
    },
    pt: {
      ...
      captureSafety:'As capturas servem apenas para ler o inventário. Ações, combate e a pontuação VS são tratados separadamente.',
      ignoredActions:'Ações/combates detetados foram ignorados automaticamente. Já estão incluídos na pontuação VS do passo seguinte.',
      ignoredLow:'Leitura(s) abaixo de 70% foram ignoradas. Usa uma captura mais nítida ou a entrada manual.',
      ignoredImplausible:'Valores claramente anormais foram ignorados para evitar um cálculo errado.',
      safeContinue:'Continuar com os valores seguros →',
      noSafeValues:'Nenhum recurso de inventário suficientemente seguro foi reconhecido. Adiciona uma captura mais nítida ou usa a entrada manual.'
    }
  };
  ...
  // OCR safety: only true inventory resources may be imported automatically.
  // Actions/combat values and "points gained" are deliberately excluded.
  const OCR_NON_INVENTORY_LABELS = new Set([
    ...ACTION_LABELS,
    'skillChipPoints'
  ]);

  function ocrItemForTarget(key){
    if(!key || key==='__currentPoints')return null;
    return findItem(key);
  }

  function isOcrInventoryTarget(key){
    const i=ocrItemForTarget(key);
    if(!i)return false;
    if(OCR_NON_INVENTORY_LABELS.has(i.labelKey))return false;
    if(/^packDiamondsD/.test(String(i.stockKey||'')))return false;
    return true;
  }

  function isPlausibleOcrValue(r){
    const i=ocrItemForTarget(r?.target);
    const value=Math.max(0,Number(r?.value)||0);
    if(!i || value<=0)return false;
    // Speed-up quantities are stored in minutes. 5,000,000 min is already
    // over 9 years and acts as a generous anti-OCR-error ceiling.
    if(i.speedup && value>5_000_000)return false;
    return Number.isFinite(value);
  }

  function ocrRowStatus(r){
    if(!r?.target || !(Number(r.value)>0))return 'invalid';
    if(!isOcrInventoryTarget(r.target))return 'action';
    if(Number(r.confidence||0)<OCR_SAFE)return 'low';
    if(!isPlausibleOcrValue(r))return 'implausible';
    return 'safe';
  }

  function ocrBuckets(){
    const rows=(typeof ocrRows!=='undefined'&&Array.isArray(ocrRows))?ocrRows:[];
    const out={safe:[],action:[],low:[],implausible:[]};
    rows.forEach((r,index)=>{
      const status=ocrRowStatus(r);
      if(out[status])out[status].push({r,index});
    });
    return out;
  }
  ...
  function css(){
    if(document.getElementById('gomo-v315-style'))return;
    const s=document.createElement('style');s.id='gomo-v315-style';
    s.textContent=`
      #plannerView.gomo-v315-simple > :not(#gomoV315Guide){display:none!important}
      #gomoV300Assistant,#gomoV310Guide,#gomoV311Guide,#gomoV312Guide,#gomoV313Guide,#gomoV314Guide{display:none!important}
      #gomoV315Guide{display:block!important;position:relative!important;z-index:2147483000!important;pointer-events:auto!important}
      #gomoV315Guide *{pointer-events:auto}
      body.gomo-v315-simple .main-nav{display:none!important}
      body.gomo-v315-simple .nav-btn{display:none!important}
      .v315 button,.v315 select,.v315 input,.v315 summary{touch-action:manipulation;-webkit-tap-highlight-color:rgba(255,255,255,.08)}
      /* Compatibilité v3.00 : sa règle !important masquait le nouveau guide. */
      #plannerView.gomo-v300-simple.gomo-v315-simple > #gomoV315Guide{display:block!important}
      #plannerView.gomo-v300-simple.gomo-v315-simple > #gomoV300Assistant{display:none!important}
      .v315{max-width:760px;margin:0 auto;padding:8px 0 30px;color:#f7fbff}
      .v315-card{border:2px solid rgba(66,194,244,.58);border-radius:28px;background:linear-gradient(150deg,rgba(15,50,75,.98),rgba(5,26,43,.99));padding:20px;box-shadow:0 18px 50px rgba(0,0,0,.22)}
      .v315-center{text-align:center}.v315-brand{font-size:.78rem;font-weight:900;letter-spacing:.18em;color:#80dbff;margin:0 0 8px}.v315 h2{font-size:clamp(2rem,8vw,3rem);line-height:1.02;margin:0 0 12px}.v315 h3{font-size:1.45rem;line-height:1.15;margin:0 0 8px}.v315 p{line-height:1.52;margin:0;color:#cbe5f2}
      .v315-promise{font-size:1.12rem;max-width:520px;margin:0 auto!important}.v315-help{font-size:.9rem;margin-top:9px!important;color:#a9cad9!important}
      .v315-primary,.v315-secondary,.v315-ghost{width:100%;border:0;border-radius:20px;min-height:62px;padding:14px 18px;font:inherit;font-size:1.08rem;font-weight:900;cursor:pointer}
      .v315-primary{background:#26a7df;color:#fff;box-shadow:0 10px 24px rgba(38,167,223,.20)}.v315-secondary{background:#0c2b40;color:#d8f3ff;border:1px solid rgba(93,201,244,.38)}.v315-ghost{min-height:46px;background:transparent;color:#9dddf8;border:1px solid rgba(93,201,244,.22);font-size:.9rem}
      .v315-btns{display:grid;gap:10px;margin-top:18px}.v315-split{display:grid;grid-template-columns:1fr 1fr;gap:10px}.v315-split>.v315-secondary,.v315-split>.v315-primary{min-height:54px;font-size:.95rem}
      .v315-progress{display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;margin-bottom:14px}.v315-progress small{font-weight:900;color:#9dddf8}.v315-track{height:8px;border-radius:999px;background:#082337;overflow:hidden}.v315-fill{height:100%;background:#29a9df;border-radius:999px;transition:width .2s}
      .v315-day{display:grid;place-items:center;min-height:140px;border-radius:23px;margin-top:16px;background:rgba(37,168,224,.10);border:1px solid rgba(85,208,255,.28)}.v315-day strong{font-size:1.8rem;text-align:center;padding:10px}
      .v315-field{display:grid;gap:7px;margin-top:16px;text-align:left}.v315-field span{font-size:.86rem;font-weight:900;color:#c6e7f6}.v315-field input,.v315-field select,.v315-number{width:100%;box-sizing:border-box;border:2px solid rgba(104,198,235,.46);border-radius:18px;background:#071f31;color:#fff;padding:15px 16px;font:inherit;font-size:1.08rem;font-weight:900;min-height:56px}
      .v315-choices{display:grid;gap:12px;margin-top:18px}.v315-choice{display:grid;grid-template-columns:48px 1fr;gap:12px;align-items:center;border:2px solid rgba(77,192,235,.42);border-radius:22px;background:#092a40;color:#fff;padding:15px;text-align:left;font:inherit;font-weight:900;font-size:1.06rem}.v315-choice b{font-size:1.55rem}.v315-choice small{display:block;color:#a9cad9;font-size:.78rem;font-weight:700;margin-top:3px}
      .v315-upload{display:grid;place-items:center;min-height:92px;border:2px dashed rgba(83,208,255,.6);border-radius:22px;background:rgba(38,167,223,.08);color:#dcf7ff;font-size:1.08rem;font-weight:900;margin-top:16px;cursor:pointer}.v315-upload input{display:none}
      .v315-ocr-note{margin-top:14px;padding:12px 13px;border-radius:15px;background:rgba(48,172,222,.10);border:1px solid rgba(84,204,248,.28);color:#c9efff;font-size:.82rem;font-weight:800;line-height:1.42}
      .v315-ocr-warning{margin-top:10px;padding:12px 13px;border-radius:15px;background:rgba(255,176,64,.10);border:1px solid rgba(255,193,99,.34);color:#ffd596;font-size:.82rem;font-weight:800;line-height:1.42}
      .v315-review{display:grid;gap:9px;margin-top:15px}.v315-row{display:grid;grid-template-columns:1fr 128px;gap:10px;align-items:center;border:1px solid rgba(108,190,225,.20);border-radius:17px;background:#09263a;padding:11px 12px}.v315-row strong{display:block;font-size:.9rem}.v315-row small{color:#85bed8}.v315-row input{width:100%;box-sizing:border-box;border:1px solid rgba(97,198,237,.35);border-radius:12px;background:#061c2c;color:#fff;padding:10px;font:inherit;font-weight:900}
      .v315-manual{display:grid;gap:8px;margin-top:15px}.v315-manual-row{display:grid;grid-template-columns:36px 1fr 118px;gap:9px;align-items:center;border-bottom:1px solid rgba(114,192,224,.14);padding:9px 0}.v315-manual-row .ico{font-size:1.25rem}.v315-manual-row strong{font-size:.9rem}.v315-manual-row small{display:block;color:#88b6ca;font-size:.72rem}.v315-manual-row input{width:100%;box-sizing:border-box;border:1px solid rgba(97,198,237,.35);border-radius:12px;background:#061c2c;color:#fff;padding:10px;font:inherit;font-weight:900}
      .v315-score{font-size:1.55rem!important;text-align:center;letter-spacing:.02em}
      .v315-stat{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:end;border-radius:20px;background:#08253a;border:1px solid rgba(95,191,229,.22);padding:14px;margin-top:10px}.v315-stat span{color:#9bc8da;font-size:.8rem;font-weight:800}.v315-stat strong{font-size:1.32rem}.v315-stat.gold strong{color:#ffd36a}
      .v315-alert{border-radius:20px;padding:15px;margin-top:14px;font-weight:900;line-height:1.45}.v315-ok{background:rgba(55,191,122,.13);border:1px solid rgba(92,222,153,.35);color:#9bf0bf}.v315-warn{background:rgba(255,173,65,.10);border:1px solid rgba(255,193,99,.34);color:#ffd08d}
      .v315-steps{counter-reset:plan;display:grid;gap:10px;margin-top:14px}.v315-planline{counter-increment:plan;display:grid;grid-template-columns:42px 1fr auto;gap:10px;align-items:center;border-radius:18px;background:#08253a;border:1px solid rgba(94,190,227,.20);padding:12px}.v315-planline:before{content:counter(plan);display:grid;place-items:center;width:34px;height:34px;border-radius:50%;background:#27a7df;color:#fff;font-weight:900}.v315-planline strong{display:block}.v315-planline small{display:block;color:#91bed1;margin-top:3px}.v315-gain{font-weight:900;color:#83ddff;white-space:nowrap}
      .v315-stop{font-size:1.08rem;text-align:center;margin-top:16px!important;color:#a6efc3!important;font-weight:900}
      .v315-done-icon{font-size:3rem;margin-bottom:8px}.v315-version{text-align:center;color:#6f9aae;font-size:.72rem;margin-top:12px}
      .v315-advanced{margin-top:14px;border-top:1px solid rgba(109,189,222,.17);padding-top:12px}.v315-advanced summary{cursor:pointer;color:#7fcce9;font-weight:800;font-size:.82rem}.v315-advanced p{font-size:.78rem;margin-top:8px!important}
      .v315-lang{margin:16px auto 0;max-width:240px}.v315-lang select{width:100%;border:1px solid rgba(96,198,237,.30);border-radius:14px;background:#071f31;color:#dff7ff;padding:10px 12px;font:inherit;font-weight:800}
      @media(max-width:520px){.v315-card{padding:18px 14px;border-radius:24px}.v315-split{grid-template-columns:1fr}.v315-row{grid-template-columns:1fr 110px}.v315-manual-row{grid-template-columns:32px 1fr 105px}.v315-planline{grid-template-columns:38px 1fr}.v315-gain{grid-column:2;text-align:left}}
    `;
  ...
  function ensure(){
    ...
    ['gomoV300Assistant','gomoV310Guide','gomoV311Guide','gomoV312Guide','gomoV313Guide','gomoV314Guide'].forEach(id=>{
      const old=document.getElementById(id);
      if(old) old.remove();
    });
    pv.classList.remove('gomo-v300-simple','gomo-v310-simple','gomo-v311-simple','gomo-v312-simple','gomo-v313-simple','gomo-v314-simple');
    let root=document.getElementById('gomoV315Guide');
    if(!root){root=document.createElement('section');root.id='gomoV315Guide';pv.insertBefore(root,pv.firstChild);}
    pv.classList.toggle('gomo-v315-simple',!ui.advanced);
    document.body?.classList.toggle('gomo-v315-simple',!ui.advanced);
    if(!ui.advanced) pv.classList.remove('gomo-v300-simple','gomo-v310-simple','gomo-v311-simple','gomo-v312-simple','gomo-v313-simple','gomo-v314-simple');
    return root;
  }
  ...
  function progress(step){
    ...
    return `<div class="v315-progress"><small>${esc(tr('step'))} ${Math.max(1,step)}</small><div class="v315-track"><div class="v315-fill" style="width:${pct}%"></div></div><small>4</small></div>`;
  }

  function backButton(to){
    return `<button class="v315-ghost" type="button" data-go="${to}">← ${esc(tr('back'))}</button>`;
  }

  function advancedBlock(){
    return `<details class="v315-advanced"><summary>⚙️ ${esc(tr('advanced'))}</summary><p>${esc(tr('advancedText'))}</p><div class="v315-btns"><button class="v315-ghost" id="v315AdvancedToggle" type="button">${esc(ui.advanced?tr('hideOld'):tr('showOld'))}</button></div></details>`;
  }

  function render(){
    const root=ensure();if(!root)return;
    root.innerHTML=`<div class="v315">${screen()}</div>`;
    bind(root);
  }
  ...
  function screenHome(){
    ...
    return `<div class="v315-card v315-center">
      <p class="v315-brand">GoMo Forever · LAST WAR</p>
      <h2>${esc(tr('title'))}</h2>
      <p class="v315-promise">${esc(tr('promise'))}</p>
      <p class="v315-help">${esc(tr('homeHelp'))}</p>
      <div class="v315-btns"><button class="v315-primary" id="v315Start" type="button">${esc(hasProgress?tr('resume'):tr('start'))}</button></div>
      <label class="v315-lang"><select id="v315Language" aria-label="${esc(tr('language'))}">${languageOptions()}</select></label>
      ${advancedBlock()}
      <div class="v315-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  }
  ...
  function screenDay(){
    return `<div class="v315-card">
      ${progress(1)}
      <h3>${esc(tr('dayTitle'))}</h3><p>${esc(tr('dayHelp'))}</p>
      <div class="v315-day"><strong>${esc(dayName(state.selectedDay))}</strong></div>
      ${ui.changeDay?`<label class="v315-field"><span>${esc(tr('chooseDay'))}</span><select id="v315Day">${dayOptions()}</select></label>`:''}
      <div class="v315-btns">
        <button class="v315-primary" id="v315DayOk" type="button">${esc(tr('yesContinue'))}</button>
        <button class="v315-secondary" id="v315ChangeDay" type="button">${esc(ui.changeDay?tr('continue'):tr('chooseDay'))}</button>
        ${backButton(0)}
      </div>
    </div>`;
  }
  ...
  function screenResources(){
    ...
    if(ui.inputMode==='choice'){
      body=`<div class="v315-choices">
        <button class="v315-choice" id="v315CaptureChoice" type="button"><b>📷</b><span>${esc(tr('addCaptures'))}<small>${esc(tr('captureHint'))}</small></span></button>
        <button class="v315-choice" id="v315ManualChoice" type="button"><b>✍️</b><span>${esc(tr('manual'))}<small>${esc(tr('manualHelp'))}</small></span></button>
      </div>`;
    ...
    return `<div class="v315-card">
      ...
      ${body}
      <div class="v315-btns">${ui.inputMode!=='choice'?`<button class="v315-secondary" id="v315ResourceChoice" type="button">${esc(tr('change'))}</button>`:''}${backButton(1)}</div>
    </div>`;
  }

  function meaningfulOcr(){
    return ocrBuckets().safe;
  }

  function captureMarkup(){
    const buckets=ocrBuckets(),rows=buckets.safe;
    if(busy)return `<label class="v315-upload">${esc(tr('reading'))}</label>`;

    const warningBits=[];
    if(buckets.action.length)warningBits.push(`<div class="v315-ocr-note">🛡️ ${esc(tr('ignoredActions'))}</div>`);
    if(buckets.low.length)warningBits.push(`<div class="v315-ocr-warning">⚠️ ${buckets.low.length} — ${esc(tr('ignoredLow'))}</div>`);
    if(buckets.implausible.length)warningBits.push(`<div class="v315-ocr-warning">⚠️ ${buckets.implausible.length} — ${esc(tr('ignoredImplausible'))}</div>`);
    const warnings=warningBits.join('');

    const anyDetected=rows.length||buckets.action.length||buckets.low.length||buckets.implausible.length;
    if(!anyDetected){
      return `<div class="v315-ocr-note">🛡️ ${esc(tr('captureSafety'))}</div>
        <label class="v315-upload">📷 ${esc(tr('addCaptures'))}<input id="v315Files" type="file" accept="image/*" multiple></label>
        <p class="v315-help">${esc(tr('captureHint'))}</p>`;
    }

    if(!rows.length){
      return `<div class="v315-ocr-note">🛡️ ${esc(tr('captureSafety'))}</div>
        ${warnings}
        <div class="v315-ocr-warning">${esc(tr('noSafeValues'))}</div>
        <div class="v315-btns"><label class="v315-secondary" style="display:grid;place-items:center;cursor:pointer">📷 ${esc(tr('addCaptures'))}<input id="v315Files" type="file" accept="image/*" multiple style="display:none"></label></div>`;
    }

    return `<div class="v315-ocr-note">🛡️ ${esc(tr('captureSafety'))}</div>
      ${warnings}
      <h3 style="margin-top:18px">${esc(tr('recognized'))}</h3>
      <p class="v315-help">${esc(tr('recognizedHelp'))}</p>
      <div class="v315-review">${rows.map(({r,index})=>`<label class="v315-row"><span><strong>${esc(targetLabel(r.target))}</strong><small>✅ ${num(r.confidence||0)}% ${esc(tr('confidence'))}</small></span><input type="number" inputmode="decimal" min="0" data-ocr="${index}" value="${Number(r.value)||0}"></label>`).join('')}</div>
      <div class="v315-btns"><button class="v315-primary" id="v315ApplyOcr" type="button">${esc(tr('safeContinue'))}</button><label class="v315-secondary" style="display:grid;place-items:center;cursor:pointer">📷 ${esc(tr('addCaptures'))}<input id="v315Files" type="file" accept="image/*" multiple style="display:none"></label></div>`;
  }

  function manualMarkup(){
    const rows=todayResources();
    return `<h3 style="margin-top:18px">${esc(tr('manualTitle'))}</h3><p class="v315-help">${esc(tr('manualHelp'))}</p>
      <div class="v315-manual">${rows.map(i=>`<label class="v315-manual-row"><span class="ico">${icon(i)}</span><span><strong>${esc(label(i))}</strong><small>${esc(unit(i)||tr('quantity'))}</small></span><input type="number" inputmode="decimal" min="0" data-stock="${esc(i.stockKey)}" value="${stock(i)}"></label>`).join('')}</div>
      <div class="v315-btns"><button class="v315-primary" id="v315ManualSave" type="button">${esc(tr('saveContinue'))}</button></div>`;
  }
  ...
  function screenScore(){
    ...
    return `<div class="v315-card">
      ...
      <label class="v315-field"><span>${esc(tr('currentScore'))}</span><input class="v315-score" id="v315Score" type="number" inputmode="numeric" min="0" value="${cur||''}" placeholder="${esc(tr('scorePlaceholder'))}"></label>
      <div class="v315-btns"><button class="v315-primary" id="v315MakePlan" type="button">${esc(tr('makePlan'))}</button>${backButton(2)}</div>
    </div>`;
  }
  ...
  function screenPlan(){
    ...
    const lines=p.resources.map(r=>`<div class="v315-planline"><span><strong>${esc(label(r.item))}</strong><small>${esc(formatQty(r.qty,r.item))}</small></span><span class="v315-gain">+${compact(r.points)}</span></div>`).join('');
    const status=p.current>=MINIMUM
      ? `<div class="v315-alert v315-ok">${esc(tr('noSpend'))}</div>`
      : p.reached
        ? `<div class="v315-alert v315-ok">${esc(tr('stop'))}</div>`
        : `<div class="v315-alert v315-warn">${esc(tr('insufficient'))}<br>${esc(tr('missing'))} ${compact(p.missingMinimum)}.</div>`;

    return `<div class="v315-card">
      ${progress(4)}
      <h3>${esc(tr('planTitle'))}</h3>
      <div class="v315-stat"><span>${esc(tr('already'))}</span><strong>${compact(p.current)}</strong></div>
      ${p.current<MINIMUM?`<div class="v315-stat gold"><span>${esc(tr('missing'))}</span><strong>${compact(need)}</strong></div>`:''}
      ${p.resources.length?`<h3 style="margin-top:20px">${esc(tr('exact'))}</h3><div class="v315-steps">${lines}</div>`:''}
      <div class="v315-stat gold"><span>${esc(tr('estimated'))}</span><strong>${compact(p.finalPoints)}</strong></div>
      <div class="v315-split" style="margin-top:10px"><div class="v315-stat"><span>${esc(tr('minimum'))}</span><strong>7,2 M</strong></div><div class="v315-stat"><span>${esc(tr('target'))}</span><strong>7,3 M</strong></div></div>
      ${status}
      <div class="v315-btns">
        ${p.resources.length?`<button class="v315-secondary" id="v315Copy" type="button">📋 ${esc(tr('copy'))}</button>`:''}
        <button class="v315-primary" id="v315Done" type="button">${esc(tr('done'))}</button>
        <div class="v315-split"><button class="v315-ghost" type="button" data-go="2">${esc(tr('editResources'))}</button><button class="v315-ghost" type="button" data-go="3">${esc(tr('editScore'))}</button></div>
      </div>
      ${advancedBlock()}
      <div class="v315-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  ...
  function screenFinished(){
    ...
    return `<div class="v315-card v315-center">
      <div class="v315-done-icon">✅</div><h2>${esc(tr('finishedTitle'))}</h2><p class="v315-promise">${esc(tr('finishedText'))}</p>
      <label class="v315-field"><span>${esc(tr('actualScore'))}</span><input class="v315-score" id="v315ActualScore" type="number" inputmode="numeric" min="0" value="${actual||''}"></label>
      <div class="v315-btns"><button class="v315-primary" id="v315SaveActual" type="button">${esc(tr('saveActual'))}</button><button class="v315-secondary" id="v315Restart" type="button">${esc(tr('reset'))}</button></div>
      ${advancedBlock()}
      <div class="v315-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  }
  ...
  function applyOcr(root){
    ...
    for(const inp of inputs){
      ...
      // Final safety gate: even if a forbidden row somehow reaches the DOM,
      // it can never be written to inventory.
      if(!r?.target || ocrRowStatus(r)!=='safe')continue;
      const v=Math.max(0,Number(inp.value)||0);
      if(v<=0)continue;
      grouped.set(r.target,Math.max(grouped.get(r.target)||0,v));
    }

    if(!grouped.size){toast(tr('noSafeValues'));return;}
  ...
  function saveScore(root){
    const input=root.querySelector('#v315Score');const n=Math.max(0,Number(input?.value)||0);
  ...
  function saveActual(root){
    const n=Math.max(0,Number(root.querySelector('#v315ActualScore')?.value)||0);
  ...
  function bind(root){
    ...
    on('#v315Start','onclick',()=>goTo(1));

    on('#v315DayOk','onclick',()=>{
      ui.step=2;ui.inputMode='choice';ui.changeDay=false;saveUi();render();
    });

    on('#v315ChangeDay','onclick',()=>{
      if(ui.changeDay){ui.step=2;ui.inputMode='choice';ui.changeDay=false;}
      else ui.changeDay=true;
      saveUi();render();
    });

    on('#v315CaptureChoice','onclick',()=>{ui.inputMode='capture';saveUi();render();});
    on('#v315ManualChoice','onclick',()=>{ui.inputMode='manual';saveUi();render();});
    on('#v315ResourceChoice','onclick',()=>{ui.inputMode='choice';saveUi();render();});
    on('#v315ApplyOcr','onclick',()=>applyOcr(root));
    on('#v315ManualSave','onclick',()=>saveManual(root));
    on('#v315MakePlan','onclick',()=>saveScore(root));
    on('#v315Copy','onclick',()=>void copyPlan());
    on('#v315Done','onclick',()=>markDone());
    on('#v315SaveActual','onclick',()=>saveActual(root));
    on('#v315Restart','onclick',()=>restart());

    on('#v315AdvancedToggle','onclick',()=>{
      ui.advanced=!ui.advanced;
      saveUi();
      render();
    });

    on('#v315Language','onchange',e=>setLanguage(e.target.value));

    on('#v315Day','onchange',e=>{
    ...
    on('#v315Files','onchange',e=>void handleFiles(e.target.files));
  }
  ...
  // v3.15 : démarrage unique, sans observateur de classes et sans monkey-patch.
  // Le guide est rendu après les correctifs existants et garde ses propres gestionnaires de boutons.
  function start(){
    refresh();
    document.documentElement.setAttribute('data-gomo-v315-ready','1');
  }
