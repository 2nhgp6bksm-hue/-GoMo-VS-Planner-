...
/*
  GoMo VS Planner v3.12 — Version stable directe
...
(() => {
  ...
  const KEY = 'gomo_vs_planner_v312';
  const OCR_SAFE = 70;
  ...
  function css(){
    if(document.getElementById('gomo-v312-style'))return;
    const s=document.createElement('style');s.id='gomo-v312-style';
    s.textContent=`
      #plannerView.gomo-v312-simple > :not(#gomoV312Guide){display:none!important}
      #gomoV300Assistant{display:none!important}
      #gomoV312Guide{display:block!important}
      /* Compatibilité v3.00 : sa règle !important masquait le nouveau guide. */
      #plannerView.gomo-v300-simple.gomo-v312-simple > #gomoV312Guide{display:block!important}
      #plannerView.gomo-v300-simple.gomo-v312-simple > #gomoV300Assistant{display:none!important}
      .v312{max-width:760px;margin:0 auto;padding:8px 0 30px;color:#f7fbff}
      .v312-card{border:2px solid rgba(66,194,244,.58);border-radius:28px;background:linear-gradient(150deg,rgba(15,50,75,.98),rgba(5,26,43,.99));padding:20px;box-shadow:0 18px 50px rgba(0,0,0,.22)}
      .v312-center{text-align:center}.v312-brand{font-size:.78rem;font-weight:900;letter-spacing:.18em;color:#80dbff;margin:0 0 8px}.v312 h2{font-size:clamp(2rem,8vw,3rem);line-height:1.02;margin:0 0 12px}.v312 h3{font-size:1.45rem;line-height:1.15;margin:0 0 8px}.v312 p{line-height:1.52;margin:0;color:#cbe5f2}
      .v312-promise{font-size:1.12rem;max-width:520px;margin:0 auto!important}.v312-help{font-size:.9rem;margin-top:9px!important;color:#a9cad9!important}
      .v312-primary,.v312-secondary,.v312-ghost{width:100%;border:0;border-radius:20px;min-height:62px;padding:14px 18px;font:inherit;font-size:1.08rem;font-weight:900;cursor:pointer}
      .v312-primary{background:#26a7df;color:#fff;box-shadow:0 10px 24px rgba(38,167,223,.20)}.v312-secondary{background:#0c2b40;color:#d8f3ff;border:1px solid rgba(93,201,244,.38)}.v312-ghost{min-height:46px;background:transparent;color:#9dddf8;border:1px solid rgba(93,201,244,.22);font-size:.9rem}
      .v312-btns{display:grid;gap:10px;margin-top:18px}.v312-split{display:grid;grid-template-columns:1fr 1fr;gap:10px}.v312-split>.v312-secondary,.v312-split>.v312-primary{min-height:54px;font-size:.95rem}
      .v312-progress{display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center;margin-bottom:14px}.v312-progress small{font-weight:900;color:#9dddf8}.v312-track{height:8px;border-radius:999px;background:#082337;overflow:hidden}.v312-fill{height:100%;background:#29a9df;border-radius:999px;transition:width .2s}
      .v312-day{display:grid;place-items:center;min-height:140px;border-radius:23px;margin-top:16px;background:rgba(37,168,224,.10);border:1px solid rgba(85,208,255,.28)}.v312-day strong{font-size:1.8rem;text-align:center;padding:10px}
      .v312-field{display:grid;gap:7px;margin-top:16px;text-align:left}.v312-field span{font-size:.86rem;font-weight:900;color:#c6e7f6}.v312-field input,.v312-field select,.v312-number{width:100%;box-sizing:border-box;border:2px solid rgba(104,198,235,.46);border-radius:18px;background:#071f31;color:#fff;padding:15px 16px;font:inherit;font-size:1.08rem;font-weight:900;min-height:56px}
      .v312-choices{display:grid;gap:12px;margin-top:18px}.v312-choice{display:grid;grid-template-columns:48px 1fr;gap:12px;align-items:center;border:2px solid rgba(77,192,235,.42);border-radius:22px;background:#092a40;color:#fff;padding:15px;text-align:left;font:inherit;font-weight:900;font-size:1.06rem}.v312-choice b{font-size:1.55rem}.v312-choice small{display:block;color:#a9cad9;font-size:.78rem;font-weight:700;margin-top:3px}
      .v312-upload{display:grid;place-items:center;min-height:92px;border:2px dashed rgba(83,208,255,.6);border-radius:22px;background:rgba(38,167,223,.08);color:#dcf7ff;font-size:1.08rem;font-weight:900;margin-top:16px;cursor:pointer}.v312-upload input{display:none}
      .v312-review{display:grid;gap:9px;margin-top:15px}.v312-row{display:grid;grid-template-columns:1fr 128px;gap:10px;align-items:center;border:1px solid rgba(108,190,225,.20);border-radius:17px;background:#09263a;padding:11px 12px}.v312-row strong{display:block;font-size:.9rem}.v312-row small{color:#85bed8}.v312-row input{width:100%;box-sizing:border-box;border:1px solid rgba(97,198,237,.35);border-radius:12px;background:#061c2c;color:#fff;padding:10px;font:inherit;font-weight:900}
      .v312-manual{display:grid;gap:8px;margin-top:15px}.v312-manual-row{display:grid;grid-template-columns:36px 1fr 118px;gap:9px;align-items:center;border-bottom:1px solid rgba(114,192,224,.14);padding:9px 0}.v312-manual-row .ico{font-size:1.25rem}.v312-manual-row strong{font-size:.9rem}.v312-manual-row small{display:block;color:#88b6ca;font-size:.72rem}.v312-manual-row input{width:100%;box-sizing:border-box;border:1px solid rgba(97,198,237,.35);border-radius:12px;background:#061c2c;color:#fff;padding:10px;font:inherit;font-weight:900}
      .v312-score{font-size:1.55rem!important;text-align:center;letter-spacing:.02em}
      .v312-stat{display:grid;grid-template-columns:1fr auto;gap:12px;align-items:end;border-radius:20px;background:#08253a;border:1px solid rgba(95,191,229,.22);padding:14px;margin-top:10px}.v312-stat span{color:#9bc8da;font-size:.8rem;font-weight:800}.v312-stat strong{font-size:1.32rem}.v312-stat.gold strong{color:#ffd36a}
      .v312-alert{border-radius:20px;padding:15px;margin-top:14px;font-weight:900;line-height:1.45}.v312-ok{background:rgba(55,191,122,.13);border:1px solid rgba(92,222,153,.35);color:#9bf0bf}.v312-warn{background:rgba(255,173,65,.10);border:1px solid rgba(255,193,99,.34);color:#ffd08d}
      .v312-steps{counter-reset:plan;display:grid;gap:10px;margin-top:14px}.v312-planline{counter-increment:plan;display:grid;grid-template-columns:42px 1fr auto;gap:10px;align-items:center;border-radius:18px;background:#08253a;border:1px solid rgba(94,190,227,.20);padding:12px}.v312-planline:before{content:counter(plan);display:grid;place-items:center;width:34px;height:34px;border-radius:50%;background:#27a7df;color:#fff;font-weight:900}.v312-planline strong{display:block}.v312-planline small{display:block;color:#91bed1;margin-top:3px}.v312-gain{font-weight:900;color:#83ddff;white-space:nowrap}
      .v312-stop{font-size:1.08rem;text-align:center;margin-top:16px!important;color:#a6efc3!important;font-weight:900}
      .v312-done-icon{font-size:3rem;margin-bottom:8px}.v312-version{text-align:center;color:#6f9aae;font-size:.72rem;margin-top:12px}
      .v312-advanced{margin-top:14px;border-top:1px solid rgba(109,189,222,.17);padding-top:12px}.v312-advanced summary{cursor:pointer;color:#7fcce9;font-weight:800;font-size:.82rem}.v312-advanced p{font-size:.78rem;margin-top:8px!important}
      .v312-lang{margin:16px auto 0;max-width:240px}.v312-lang select{width:100%;border:1px solid rgba(96,198,237,.30);border-radius:14px;background:#071f31;color:#dff7ff;padding:10px 12px;font:inherit;font-weight:800}
      @media(max-width:520px){.v312-card{padding:18px 14px;border-radius:24px}.v312-split{grid-template-columns:1fr}.v312-row{grid-template-columns:1fr 110px}.v312-manual-row{grid-template-columns:32px 1fr 105px}.v312-planline{grid-template-columns:38px 1fr}.v312-gain{grid-column:2;text-align:left}}
    `;
  ...
  function ensure(){
    ...
    ['gomoV300Assistant','gomoV310Guide','gomoV311Guide'].forEach(id=>{
      const old=document.getElementById(id);
      if(old) old.remove();
    });
    pv.classList.remove('gomo-v300-simple','gomo-v310-simple','gomo-v311-simple');
    let root=document.getElementById('gomoV312Guide');
    if(!root){root=document.createElement('section');root.id='gomoV312Guide';pv.insertBefore(root,pv.firstChild);}
    pv.classList.toggle('gomo-v312-simple',!ui.advanced);
    if(!ui.advanced) pv.classList.remove('gomo-v300-simple');
    return root;
  }
  ...
  function progress(step){
    ...
    return `<div class="v312-progress"><small>${esc(tr('step'))} ${Math.max(1,step)}</small><div class="v312-track"><div class="v312-fill" style="width:${pct}%"></div></div><small>4</small></div>`;
  }

  function backButton(to){
    return `<button class="v312-ghost" type="button" data-go="${to}">← ${esc(tr('back'))}</button>`;
  }

  function advancedBlock(){
    return `<details class="v312-advanced"><summary>⚙️ ${esc(tr('advanced'))}</summary><p>${esc(tr('advancedText'))}</p><div class="v312-btns"><button class="v312-ghost" id="v312AdvancedToggle" type="button">${esc(ui.advanced?tr('hideOld'):tr('showOld'))}</button></div></details>`;
  }

  function render(){
    const root=ensure();if(!root)return;
    root.innerHTML=`<div class="v312">${screen()}</div>`;
    bind(root);
  }
  ...
  function screenHome(){
    ...
    return `<div class="v312-card v312-center">
      <p class="v312-brand">GoMo Forever · LAST WAR</p>
      <h2>${esc(tr('title'))}</h2>
      <p class="v312-promise">${esc(tr('promise'))}</p>
      <p class="v312-help">${esc(tr('homeHelp'))}</p>
      <div class="v312-btns"><button class="v312-primary" id="v312Start" type="button">${esc(hasProgress?tr('resume'):tr('start'))}</button></div>
      <label class="v312-lang"><select id="v312Language" aria-label="${esc(tr('language'))}">${languageOptions()}</select></label>
      ${advancedBlock()}
      <div class="v312-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  }
  ...
  function screenDay(){
    return `<div class="v312-card">
      ${progress(1)}
      <h3>${esc(tr('dayTitle'))}</h3><p>${esc(tr('dayHelp'))}</p>
      <div class="v312-day"><strong>${esc(dayName(state.selectedDay))}</strong></div>
      ${ui.changeDay?`<label class="v312-field"><span>${esc(tr('chooseDay'))}</span><select id="v312Day">${dayOptions()}</select></label>`:''}
      <div class="v312-btns">
        <button class="v312-primary" id="v312DayOk" type="button">${esc(tr('yesContinue'))}</button>
        <button class="v312-secondary" id="v312ChangeDay" type="button">${esc(ui.changeDay?tr('continue'):tr('chooseDay'))}</button>
        ${backButton(0)}
      </div>
    </div>`;
  }
  ...
  function screenResources(){
    ...
    if(ui.inputMode==='choice'){
      body=`<div class="v312-choices">
        <button class="v312-choice" id="v312CaptureChoice" type="button"><b>📷</b><span>${esc(tr('addCaptures'))}<small>${esc(tr('captureHint'))}</small></span></button>
        <button class="v312-choice" id="v312ManualChoice" type="button"><b>✍️</b><span>${esc(tr('manual'))}<small>${esc(tr('manualHelp'))}</small></span></button>
      </div>`;
    ...
    return `<div class="v312-card">
      ...
      ${body}
      <div class="v312-btns">${ui.inputMode!=='choice'?`<button class="v312-secondary" id="v312ResourceChoice" type="button">${esc(tr('change'))}</button>`:''}${backButton(1)}</div>
    </div>`;
  }
  ...
  function captureMarkup(){
    ...
    if(busy)return `<label class="v312-upload">${esc(tr('reading'))}</label>`;
    if(!rows.length)return `<label class="v312-upload">📷 ${esc(tr('addCaptures'))}<input id="v312Files" type="file" accept="image/*" multiple></label><p class="v312-help">${esc(tr('captureHint'))}</p>`;
    return `<h3 style="margin-top:18px">${esc(tr('recognized'))}</h3><p class="v312-help">${esc(tr('recognizedHelp'))}</p>
      <div class="v312-review">${rows.map(({r,index})=>`<label class="v312-row"><span><strong>${esc(targetLabel(r.target))}</strong><small>${num(r.confidence||0)}% ${esc(tr('confidence'))}</small></span><input type="number" inputmode="decimal" min="0" data-ocr="${index}" value="${Number(r.value)||0}"></label>`).join('')}</div>
      <div class="v312-btns"><button class="v312-primary" id="v312ApplyOcr" type="button">${esc(tr('validate'))}</button><label class="v312-secondary" style="display:grid;place-items:center;cursor:pointer">📷 ${esc(tr('addCaptures'))}<input id="v312Files" type="file" accept="image/*" multiple style="display:none"></label></div>`;
  }

  function manualMarkup(){
    const rows=todayResources();
    return `<h3 style="margin-top:18px">${esc(tr('manualTitle'))}</h3><p class="v312-help">${esc(tr('manualHelp'))}</p>
      <div class="v312-manual">${rows.map(i=>`<label class="v312-manual-row"><span class="ico">${icon(i)}</span><span><strong>${esc(label(i))}</strong><small>${esc(unit(i)||tr('quantity'))}</small></span><input type="number" inputmode="decimal" min="0" data-stock="${esc(i.stockKey)}" value="${stock(i)}"></label>`).join('')}</div>
      <div class="v312-btns"><button class="v312-primary" id="v312ManualSave" type="button">${esc(tr('saveContinue'))}</button></div>`;
  }
  ...
  function screenScore(){
    ...
    return `<div class="v312-card">
      ...
      <label class="v312-field"><span>${esc(tr('currentScore'))}</span><input class="v312-score" id="v312Score" type="number" inputmode="numeric" min="0" value="${cur||''}" placeholder="${esc(tr('scorePlaceholder'))}"></label>
      <div class="v312-btns"><button class="v312-primary" id="v312MakePlan" type="button">${esc(tr('makePlan'))}</button>${backButton(2)}</div>
    </div>`;
  }
  ...
  function screenPlan(){
    ...
    const lines=p.resources.map(r=>`<div class="v312-planline"><span><strong>${esc(label(r.item))}</strong><small>${esc(formatQty(r.qty,r.item))}</small></span><span class="v312-gain">+${compact(r.points)}</span></div>`).join('');
    const status=p.current>=MINIMUM
      ? `<div class="v312-alert v312-ok">${esc(tr('noSpend'))}</div>`
      : p.reached
        ? `<div class="v312-alert v312-ok">${esc(tr('stop'))}</div>`
        : `<div class="v312-alert v312-warn">${esc(tr('insufficient'))}<br>${esc(tr('missing'))} ${compact(p.missingMinimum)}.</div>`;

    return `<div class="v312-card">
      ${progress(4)}
      <h3>${esc(tr('planTitle'))}</h3>
      <div class="v312-stat"><span>${esc(tr('already'))}</span><strong>${compact(p.current)}</strong></div>
      ${p.current<MINIMUM?`<div class="v312-stat gold"><span>${esc(tr('missing'))}</span><strong>${compact(need)}</strong></div>`:''}
      ${p.resources.length?`<h3 style="margin-top:20px">${esc(tr('exact'))}</h3><div class="v312-steps">${lines}</div>`:''}
      <div class="v312-stat gold"><span>${esc(tr('estimated'))}</span><strong>${compact(p.finalPoints)}</strong></div>
      <div class="v312-split" style="margin-top:10px"><div class="v312-stat"><span>${esc(tr('minimum'))}</span><strong>7,2 M</strong></div><div class="v312-stat"><span>${esc(tr('target'))}</span><strong>7,3 M</strong></div></div>
      ${status}
      <div class="v312-btns">
        ${p.resources.length?`<button class="v312-secondary" id="v312Copy" type="button">📋 ${esc(tr('copy'))}</button>`:''}
        <button class="v312-primary" id="v312Done" type="button">${esc(tr('done'))}</button>
        <div class="v312-split"><button class="v312-ghost" type="button" data-go="2">${esc(tr('editResources'))}</button><button class="v312-ghost" type="button" data-go="3">${esc(tr('editScore'))}</button></div>
      </div>
      ${advancedBlock()}
      <div class="v312-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  ...
  function screenFinished(){
    ...
    return `<div class="v312-card v312-center">
      <div class="v312-done-icon">✅</div><h2>${esc(tr('finishedTitle'))}</h2><p class="v312-promise">${esc(tr('finishedText'))}</p>
      <label class="v312-field"><span>${esc(tr('actualScore'))}</span><input class="v312-score" id="v312ActualScore" type="number" inputmode="numeric" min="0" value="${actual||''}"></label>
      <div class="v312-btns"><button class="v312-primary" id="v312SaveActual" type="button">${esc(tr('saveActual'))}</button><button class="v312-secondary" id="v312Restart" type="button">${esc(tr('reset'))}</button></div>
      ${advancedBlock()}
      <div class="v312-version">${esc(tr('version'))} · v${VERSION}</div>
    </div>`;
  }
  ...
  function saveScore(root){
    const input=root.querySelector('#v312Score');const n=Math.max(0,Number(input?.value)||0);
  ...
  function saveActual(root){
    const n=Math.max(0,Number(root.querySelector('#v312ActualScore')?.value)||0);
  ...
  function bind(root){
    if(root.dataset.v312Bound==='1')return;
    root.dataset.v312Bound='1';
    root.addEventListener('click',e=>{
      const go=e.target.closest('[data-go]');if(go){ui.step=Number(go.dataset.go)||0;ui.finished=false;saveUi();render();return;}
      if(e.target.closest('#v312Start')){ui.step=1;ui.finished=false;saveUi();render();return;}
      if(e.target.closest('#v312DayOk')){ui.step=2;ui.inputMode='choice';ui.changeDay=false;saveUi();render();return;}
      if(e.target.closest('#v312ChangeDay')){
        if(ui.changeDay){ui.step=2;ui.inputMode='choice';ui.changeDay=false;}else ui.changeDay=true;
        saveUi();render();return;
      }
      if(e.target.closest('#v312CaptureChoice')){ui.inputMode='capture';saveUi();render();return;}
      if(e.target.closest('#v312ManualChoice')){ui.inputMode='manual';saveUi();render();return;}
      if(e.target.closest('#v312ResourceChoice')){ui.inputMode='choice';saveUi();render();return;}
      if(e.target.closest('#v312ApplyOcr')){applyOcr(root);return;}
      if(e.target.closest('#v312ManualSave')){saveManual(root);return;}
      if(e.target.closest('#v312MakePlan')){saveScore(root);return;}
      if(e.target.closest('#v312Copy')){void copyPlan();return;}
      if(e.target.closest('#v312Done')){markDone();return;}
      if(e.target.closest('#v312SaveActual')){saveActual(root);return;}
      if(e.target.closest('#v312Restart')){restart();return;}
      if(e.target.closest('#v312AdvancedToggle')){ui.advanced=!ui.advanced;saveUi();ensure();render();return;}
    });
    root.addEventListener('change',e=>{
      if(e.target.id==='v312Language'){setLanguage(e.target.value);return;}
      if(e.target.id==='v312Day'){
        state.selectedDay=Number(e.target.value);state.autoDay=false;planCache=null;persistBase();render();return;
      }
      if(e.target.id==='v312Files'){void handleFiles(e.target.files);return;}
    });
  }
  ...
  // Keep this guide visible after the base app or v3.00 rerenders.
  try{
    if(typeof renderAll==='function'){
      const oldRenderAll=renderAll;
      renderAll=function(){const r=oldRenderAll.apply(this,arguments);setTimeout(refresh,0);return r;};
    }
  }catch{}
  try{
    if(typeof applyTranslations==='function'){
      const oldApply=applyTranslations;
      applyTranslations=function(){const r=oldApply.apply(this,arguments);setTimeout(refresh,0);return r;};
    }
  }catch{}
  ...
  function start(){
    ...
    if(pv){
      ...
      observer.observe(pv,{attributes:true,attributeFilter:['class']});
    }
  ...
  console.info('GoMo VS Planner guide automatique',VERSION);
})();
